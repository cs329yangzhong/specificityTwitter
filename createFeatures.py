...
class ModelNewText(object):
	...
	def loadFromCSV(self,filename):
		...
		self.test = [features.ParseText(y) for y in list(original_df.loc[:,'Tweet'])]
		print(len(self.test))
	...
	def loadFromTSV(self,filename):
		...
		print(len(self.userid))
		try:
			self.time = [int(y.split()[-1]) for y in list(original_df.loc[:,'time'])]
			self.tweetid = list(original_df.loc[:,'tweet_id'])
			self.testLabels = list(original_df.loc[:,'Score'])
		except KeyError:
			pass
		
		self.test = [features.ParseText(y) for y in list(original_df.loc[:,'text'])]
		print(len(self.test))
	...
	def fNeuralVec(self):
		...
		if keys[0] not in self.featurestest:
			...
			for fid,fname in enumerate(keys):
				self.featurestest[fname] = [embeddingList[j][fid] for j in range(len(embeddingList))]
	...
	def fBrownCluster_100(self):
		...
		if keys[0] not in self.featurestest:
			...
			brownClusterList = features.brownCluster(sentlst, brownClus, cluster_2_index, 100)
			for fid,fname in enumerate(keys):
				self.featurestest[fname] = [brownClusterList[j][fid] for j in range(len(brownClusterList))]
	
	# NE and Concrete words.
	def NE_Concrete(self):
		sentlst = [features.RawSent(r) for r in self.test]
		pos_tag = features.NE_Concrete_Emo(sentlst)
		Useful_Tag = ['ORGANIZATION', "PERCENT", 'PERSON', 'DATE', 'MONEY', 'TIME', 'LOCATION', 'Concrete']
		for i in Useful_Tag:
			self._add_feature(i, pos_tag.loc[:,i])
